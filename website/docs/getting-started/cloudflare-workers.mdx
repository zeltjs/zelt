...
// Running `wrangler types` augments the global `Env` with the bindings
// declared in `wrangler.toml` — here it's a KV namespace binding named `CACHE`.
declare global {
  interface Env {
    CACHE: KVNamespace;
  }
...
`bindings.get('CACHE')` is typed as `KVNamespace` (or `D1Database`, `R2Bucket`, etc. — whatever binding you declared) without any manual typing, since it reads the type straight from the augmented global `Env`.
