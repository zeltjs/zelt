...
Koya automatically generates type-safe client types (`AppType`) and OpenAPI 3.1 specifications from your controllers — no decorators or annotations required.

## Overview

The `@koya/contract` package provides:
...
Create a `koya.config.ts` file in your project root:
...
Koya uses a "zero-annotation" approach inspired by [Scramble](https://scramble.dedoc.co/):
