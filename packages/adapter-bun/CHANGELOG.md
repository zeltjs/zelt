...
* **adapter-bun:** `onBun()` accepts a `prebuilt` option (`ZeltPrebuilt`) and
  threads it through to the runtime's `ServiceResolver.prebuilt`, so feature
  modules such as `@zeltjs/graphql` can read their generated runtime without
