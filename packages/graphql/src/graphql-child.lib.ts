...
import type { GqlSchemaRef, GraphqlResolverClass } from './graphql-metadata.lib';
...
// `name` becomes a filename under `.zelt/graphql/` and a prebuilt lookup
// key, so it's constrained to what's safe on every filesystem: no path
// separators, no reserved Windows device names.
const GRAPHQL_NAME_RE = /^[A-Za-z0-9_-]+$/;
const WINDOWS_RESERVED_NAME_RE = /^(?:con|prn|aux|nul|com[1-9]|lpt[1-9])$/i;

/** @throws {Error} */
const assertValidGraphqlName = (name: string): void => {
  if (GRAPHQL_NAME_RE.test(name) && !WINDOWS_RESERVED_NAME_RE.test(name)) return;
  throw new Error(
    `Invalid GraphQL endpoint name ${JSON.stringify(name)}. \`name\` must match /^[A-Za-z0-9_-]+$/ and cannot be a reserved Windows device name (CON, PRN, AUX, NUL, COM1-9, LPT1-9).`,
  );
};
...
export type GraphqlOptions = {
  ...
  // Presence selects the line for this endpoint: schema-first when set,
  // code-first otherwise. Pass the `schema` export from a codegen helper
  // generated by `zelt graphql codegen`.
...
export class GraphqlHttpFeature implements HttpMountableFeatureModule {
  ...
  constructor(private readonly options: GraphqlOptions) {
    ...
    if (options.name !== undefined) assertValidGraphqlName(options.name);
