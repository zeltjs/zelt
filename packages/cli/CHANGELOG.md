...
* **cli:** `BuildContext` gains a required `registerGeneratedFile(path)`
  field. Plugins with a `preBuild` hook should call it with the absolute
  path of every file they write. `zelt build` and `zelt dev` use the
  registrations to prune outputs a plugin no longer produces (e.g. after a
  `graphql()` endpoint is removed from the app): any file recorded by a
  previous run but not registered this run is deleted. Files under `.zelt/`
  are deleted without further checks; files elsewhere are only deleted if
  they contain the generated-file header marker (`// Generated by zelt
  build. Do not edit.` or equivalent), so handwritten files are never
  removed. The registry itself (`.zelt/outputs.json`) and the GraphQL
  codegen manifest (`.zelt/graphql-codegen.json`) are excluded from pruning.
