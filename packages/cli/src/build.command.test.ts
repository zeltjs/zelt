...
describe('runBuild', () => {
  ...
  it('prunes a generated output a plugin stops producing on the next build', async () => {
    const configWithBothFiles = `
        ...
        plugins: [
          {
            name: 'fake-graphql',
            preBuild: async (ctx) => {
              const runtimePath = join(ctx.cwd, '.zelt', 'graphql', 'admin.runtime.ts');
              await mkdir(join(ctx.cwd, '.zelt', 'graphql'), { recursive: true });
              await writeFile(runtimePath, 'export const graphqlPrebuilt = {};\\n');
              ctx.registerGeneratedFile(runtimePath);

              const checksPath = join(ctx.cwd, 'src', 'generated', 'admin.resolver-checks.ts');
              await mkdir(join(ctx.cwd, 'src', 'generated'), { recursive: true });
              await writeFile(
                checksPath,
                '// Generated by zelt build. Do not edit.\\nexport {};\\n',
              );
              ctx.registerGeneratedFile(checksPath);
            },
          },
        ],
        build: {
    ...
    `;
    await writeFile(join(testDir, 'zelt.config.ts'), configWithBothFiles);
    await runBuild(testDir, {});

    await expect(
      readFile(join(testDir, '.zelt/graphql/admin.runtime.ts'), 'utf8'),
    ).resolves.toContain('graphqlPrebuilt');
    await expect(
      readFile(join(testDir, 'src/generated/admin.resolver-checks.ts'), 'utf8'),
    ).resolves.toContain('export {}');

    // Simulates the admin endpoint being removed from the app: the plugin no
    // longer writes or registers the resolver-checks file.
    const configWithOnlyRuntimeFile = `
      import { mkdir, writeFile } from 'node:fs/promises';
      import { join } from 'node:path';
    ...
    `;
    await writeFile(join(testDir, 'zelt.config.ts'), configWithOnlyRuntimeFile);
    await runBuild(testDir, {});
