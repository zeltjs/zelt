...
describe('URI Versioning', () => {
  ...
  describe('GET /', () => {
    it('V1', async () => {
      const res = await testApp.http.request('/v1/');
      expect(res.status).toBe(200);
      expect(await res.text()).toBe('Hello World V1!');
    });

    it('V2', async () => {
      const res = await testApp.http.request('/v2/');
      expect(res.status).toBe(200);
      expect(await res.text()).toBe('Hello World V2!');
    });

    it('V3', async () => {
      const res = await testApp.http.request('/v3/');
  ...
  describe('GET /:param/hello', () => {
    it('V1', async () => {
      const res = await testApp.http.request('/v1/param/hello');
      expect(res.status).toBe(200);
      expect(await res.text()).toBe('Parameter V1: param');
    });

    it('V2', async () => {
      const res = await testApp.http.request('/v2/param/hello');
      expect(res.status).toBe(200);
      expect(await res.text()).toBe('Parameter V2: param');
    });

    it('V3', async () => {
      const res = await testApp.http.request('/v3/param/hello');
      expect(res.status).toBe(404);
    });

    it('No Version', async () => {
      const res = await testApp.http.request('/param/hello');
      expect(res.status).toBe(404);
    });
  });
  ...
  describe('GET /multiple', () => {
    it('V1', async () => {
      const res = await testApp.http.request('/v1/multiple');
      expect(res.status).toBe(200);
      expect(await res.text()).toBe('Multiple Versions 1 or 2');
    });

    it('V2', async () => {
      const res = await testApp.http.request('/v2/multiple');
      expect(res.status).toBe(200);
      expect(await res.text()).toBe('Multiple Versions 1 or 2');
    });

    it('V3', async () => {
      const res = await testApp.http.request('/v3/multiple');
      expect(res.status).toBe(404);
    });

    it('No Version', async () => {
      const res = await testApp.http.request('/multiple');
      expect(res.status).toBe(404);
    });
  });
  ...
  describe('GET /neutral', () => {
    it('No Version (version-neutral controller)', async () => {
      const res = await testApp.http.request('/neutral');
      expect(res.status).toBe(200);
  ...
  describe('GET /override', () => {
    it('V1', async () => {
      const res = await testApp.http.request('/v1/override');
      expect(res.status).toBe(200);
      expect(await res.text()).toBe('Override Version 1');
    });

    it('V2', async () => {
      const res = await testApp.http.request('/v2/override');
      expect(res.status).toBe(200);
      expect(await res.text()).toBe('Override Version 2');
    });

    it('V3', async () => {
      const res = await testApp.http.request('/v3/override');
      expect(res.status).toBe(404);
    });

    it('No Version', async () => {
      const res = await testApp.http.request('/override');
      expect(res.status).toBe(404);
    });
  });
  ...
  describe('GET /override-partial', () => {
    it('V1', async () => {
      const res = await testApp.http.request('/v1/override-partial');
      expect(res.status).toBe(200);
      expect(await res.text()).toBe('Override Partial Version 1');
    });

    it('V2', async () => {
      const res = await testApp.http.request('/v2/override-partial');
      expect(res.status).toBe(200);
      expect(await res.text()).toBe('Override Partial Version 2');
    });

    it('V3', async () => {
      const res = await testApp.http.request('/v3/override-partial');
      expect(res.status).toBe(404);
    });

    it('No Version', async () => {
      const res = await testApp.http.request('/override-partial');
      expect(res.status).toBe(404);
    });
  });
  ...
  describe('GET /foo/bar (no versioning controller)', () => {
    it('V1', async () => {
      const res = await testApp.http.request('/v1/foo/bar');
      expect(res.status).toBe(404);
    });

    it('V2', async () => {
      const res = await testApp.http.request('/v2/foo/bar');
      expect(res.status).toBe(404);
    });

    it('V3', async () => {
      const res = await testApp.http.request('/v3/foo/bar');
      expect(res.status).toBe(404);
    });

    it('No Version', async () => {
      const res = await testApp.http.request('/foo/bar');
      expect(res.status).toBe(200);
  ...
  describe('Deep nested URI versioning (/vX/api/users/:id/posts/:postId)', () => {
    it('V1 resolves both path parameters', async () => {
      const res = await testApp.http.request('/v1/api/users/42/posts/7');
      expect(res.status).toBe(200);
      expect(await res.text()).toBe('V1 user=42 post=7');
    });

    it('V2 resolves both path parameters', async () => {
      const res = await testApp.http.request('/v2/api/users/42/posts/7');
      expect(res.status).toBe(200);
      expect(await res.text()).toBe('V2 user=42 post=7');
    });

    it('V3 has no handler', async () => {
      const res = await testApp.http.request('/v3/api/users/42/posts/7');
      expect(res.status).toBe(404);
    });

    it('No Version has no handler', async () => {
      const res = await testApp.http.request('/api/users/42/posts/7');
      expect(res.status).toBe(404);
    });

    it('Missing trailing segment returns 404', async () => {
      const res = await testApp.http.request('/v1/api/users/42/posts');
      expect(res.status).toBe(404);
    });
  });
});
