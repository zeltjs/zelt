...
describe('Hello World', () => {
  ...
  it('GET /hello/async returns greeting (Promise/async)', async () => {
    const res = await testApp.http.request('/hello/async');
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body).toEqual({ message: 'Hello, World!' });
  });
  ...
  it('GET /hello/stream returns greeting (stream)', async () => {
    const res = await testApp.http.request('/hello/stream');
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body).toEqual({ message: 'Hello, World!' });
  });
  ...
  it('GET /hello/:name returns personalized greeting', async () => {
    const res = await testApp.http.request('/hello/Alice');
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it('GET /unknown returns 404', async () => {
    const res = await testApp.http.request('/unknown');
